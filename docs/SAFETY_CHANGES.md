...
### Incoming unavailable-agent fallback
The System Call answer route now follows the configured fallback_user/fallback_users chain when an agent is unavailable, including offline/busy, working-hours and browser-presence checks. Traversal visits at most 16 unique agents and releases rejected-agent locks before moving on. The selected agent/device is retained on provider retries. Forwarded browser calls validate the original DID against the authenticated call log instead of requiring the fallback agent to own that DID. This handles pre-routing unavailability; it does not implement retrying another agent after a routed leg rings unanswered. Read-only site check selected mis@sriaas.com when Jagmohan was treated as offline. 43 Python tests passed; live provider testing remains.

### Patient-aware incoming routing
Incoming calls now resolve a unique Patient via existing indexed phone fields before selecting an agent. Known Patients use Patient or CRM Lead and Patient queue mappings with matching Department and Follow up ID (plus Disease/Language for Regional), rechecked under the agent lock. Fallback agents must satisfy the same criteria. The call log links the Patient. Ambiguous phone matches hang up rather than guessing; unmatched callers retain DID/fallback routing excluding Patient-only queues. Last-ten matching is limited to +91 numbers. Existing provider retries retain their recorded route. 46 Python tests passed. Read-only Kidney/7 check matches MIS; MIS was Offline with no browser lease at verification. No paid calls or patient updates were performed.

### Unknown-caller Incoming Mapping integration
Routing now checks Patient, then the core canonical/latest CRM Lead phone lookup, then enabled Vobiz Incoming Mapping for callers absent from both. Known CRM callers use their lead owner/fallback chain and link the call to the lead. Unknown callers use only enabled listed agents, requiring console online status plus device/availability/working-hours checks. Explicitly listed Patient-queue agents may receive these new callers. Round Robin/Load Balancing ordering and assignment timestamps are retained; the authenticated answer handler serializes DID assignments with a Redis lock. Retries retain the existing call assignment. Incoming Mapping no longer requires a unique caller_id user mapping. 49 tests passed; read-only DID check found eight enabled candidates but none currently online and eligible. No calls placed or round-robin position advanced during validation.

### CRM Lead creation for assigned unknown callers
Unknown callers assigned through Incoming Mapping now use the core create_unknown_inbound_lead helper, respecting mapping status/pipeline/platform/source defaults and assigning lead_owner to the selected agent. The incoming log stores CRM Lead reference and crm_lead. Caller and DID Redis locks cover lookup through commit for this answer route; repeated provider UUIDs reuse the stored call without duplicate lead creation or assignment advancement. No lead is created when no eligible listed agent is found. Existing Patients/leads are not recreated. 49 tests passed including creation/linkage/retry regression; no live test customer records were created.

### Callback logging, exact lead defaults, and unanswered routing
Callback logging now uses a worker adapter with event_type because Frappe enqueue reserves event and does not forward it as a function argument. Deploy the core callback_logging.py adapter together with both callback enqueue callers. Existing failed jobs are not automatically replayed.

Fresh incoming leads use only the Incoming Mapping selected Default Pipeline, Default Platform, and Default Source; arbitrary first-record and AI default fallbacks were removed. No configured selections are changed by this fix.

Rejected incoming routes record a reason in Vobiz Error Log. Incoming Dial action callbacks can try another eligible agent after busy/no-answer/timeout, with separate call logs, excluded attempted agents, a maximum of eight attempts, authenticated serialized callbacks, and explicit agent-cancellation protection. Existing answered calls do not trigger fallback. Unknown callers retain the same CRM Lead across attempts; its initial assigned owner is retained.

Python regression checks cover reserved queue arguments, exact defaults, cancellation/answered-call guards, and separate fallback attempts. Live provider verification is still required for action callbacks, recordings/CDR linkage across attempts, disposition timing, and concurrent calls. Lead creation remains synchronous and callback latency needs staging measurement. These changes alone do not certify production readiness.
